		...
		while ((line = reader.readLine()) != null) {
			...
			if (OPTAB.get(parts[1]).isImperative()) {
				if (parts[1].equals("STOP")) {
					ans.append((!OPTAB.get(parts[1]).isAssemblerDirective()) ? (LC + "  ") : ("---  "));
					ans.append(OPTAB.get(parts[1])).append("\n");
					LC++;
				} else {
					...
					if (operands.length == 1) {
						...
						ans.append(OPTAB.get(operands[0])).append("  ");
						operand = operands[1];
					}
					OperandEntry entry;
					if (operand.contains("=")) {
						...
						ans.append(entry).append("\n");
					} else if (operand.matches("\\d+")) {
						ans.append(constantPair(operand)).append("\n");
			...
			if (parts[1].equals("LTORG") || parts[1].equals("END")) {
				...
				for (int j = ptr; j < littab_ptr; j++) {
					...
					ans.append(LC + "  ");
					ans.append(OPTAB.get("DC")).append("  "); 			// (DL,02)
					ans.append(constantPair(LITTAB.get(j).getLiteral()) + "\n"); 	// (C,5)
					LC++;
				}
				if (littab_ptr == ptr) {
					ans.append(LC + "  ");
					ans.append(OPTAB.get("END")).append("\n");
				}
			...
			if (parts[1].equals("START") || parts[1].equals("ORIGIN")) {
				LC = expr(parts[2]);
				ans.append((!OPTAB.get(parts[1]).isAssemblerDirective()) ? (LC + "  ") : ("---  "));
				ans.append(OPTAB.get(parts[1])).append("  "); 	// (AD,01/03)
				ans.append(constantPair(LC) + "\n"); 						// (C,200)
			}
			...
			if (parts[1].equals("EQU")) {
				...
				ans.append((!OPTAB.get(parts[1]).isAssemblerDirective()) ? (LC + "  ") : ("---  "));
				ans.append(OPTAB.get("EQU")).append("  "); 	// (AD,04)
				ans.append(constantPair(addr) + "\n"); 				// (C,202)
				LC++;
			}
